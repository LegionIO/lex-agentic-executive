...
RSpec.describe Legion::Extensions::Agentic::Executive::Planning::Helpers::PlanStore do
  ...
  describe '#create_plan' do
    ...
    it 'returns a Plan instance' do
      expect(create_basic_plan).to be_a(Legion::Extensions::Agentic::Executive::Planning::Helpers::Plan)
    end
    ...
    it 'creates PlanStep objects from hashes' do
      plan = create_basic_plan
      expect(plan.steps).to all(be_a(Legion::Extensions::Agentic::Executive::Planning::Helpers::PlanStep))
    end

    it 'accepts PlanStep objects directly' do
      step = Legion::Extensions::Agentic::Executive::Planning::Helpers::PlanStep.new(action: :direct)
  ...
  describe '#replan' do
    ...
    it 'rejects when replan limit exceeded' do
      Legion::Extensions::Agentic::Executive::Planning::Helpers::Constants::REPLAN_LIMIT.times do
        store.replan(plan_id: plan.id, new_steps: new_steps)
      end
      result = store.replan(plan_id: plan.id, new_steps: new_steps)
  ...
  describe '#trim_history' do
    it 'archives oldest plan when MAX_PLANS is exceeded' do
      max = Legion::Extensions::Agentic::Executive::Planning::Helpers::Constants::MAX_PLANS
